...
import { ipcRoutes, type IpcInput, type IpcResult, type IpcRouteKey } from './contract';

export type IpcInvoke = <K extends IpcRouteKey>(key: K, args: IpcInput<K>) => Promise<IpcResult<K>>;
...
type MethodFn<K extends IpcRouteKey> = IpcInput<K> extends undefined
  ? () => Promise<IpcResult<K>>
  : (args: IpcInput<K>) => Promise<IpcResult<K>>;
...
export const createBillmeApi = (invoke: IpcInvoke): BillmeApi => {
  ...
  for (const key of Object.keys(ipcRoutes) as IpcRouteKey[]) {
    ...
    if (hasUndefinedArgs(route.args)) {
      api[group]![method] = () => invoke(key, undefined as IpcInput<typeof key>);
    } else {
      api[group]![method] = (args: unknown) => invoke(key, args as IpcInput<typeof key>);
    }
  }
